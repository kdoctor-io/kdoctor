...
对于这种任务， kdoctor-controller 会根据 agentSpec 生成对应的 agent ，每一个 agent pod 都会向指定的 dns server 发送 dns 请求，默认并发量为 50 可覆盖多副本情况，并发量可在 kodcotr 的 configmap 中设置，并获得成功率和平均延迟。根据成功条件来判断结果是否成功。并且，可以通过聚合API获取详细的报告。
