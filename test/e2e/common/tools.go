...
func CreateApplicationDaemonSetAndService(f *frame.Framework, key metav1.ObjectMeta, app, insecure bool) error {
	...
	// daemonSet
	ds.ObjectMeta = key
	ds.Spec.Selector = &metav1.LabelSelector{MatchLabels: key.Labels}
	ds.Spec.Template.ObjectMeta = key
	...
	container.Name = key.Name
	container.Image = AgentImageRegistry + ":" + AgentImageTag
	container.ImagePullPolicy = corev1.PullIfNotPresent
	container.Command = []string{"/usr/bin/agent"}
	container.Args = []string{"--tls-ca-cert=/etc/tls/ca.crt", "--tls-ca-key=/etc/tls/ca.key"}
	if app {
		container.Args = append(container.Args, "--app-mode=true")
	}
	if !insecure {
		container.Args = append(container.Args, "--tls-insecure=true")
	}
	container.VolumeMounts = []corev1.VolumeMount{{MountPath: "/etc/tls", Name: "tls"}}

	ds.Spec.Template.Spec.Containers = append(ds.Spec.Template.Spec.Containers, container)

	// service
	svc.ObjectMeta = key
	svc.Spec.Selector = key.Labels
	...
	e := f.CreateDaemonSet(ds)
	if e != nil {
		return fmt.Errorf("create test daemonset failed ,reason=%v", e)
	}

	e = f.CreateService(svc)
	if e != nil {
		return fmt.Errorf("create test service failed ,reason=%v", e)
	}
	return nil
}
